  ...
  private async fetchValidateAndCache<TResponse>(opts: {
    ...
    if (!fetchPromise) {
      fetchPromise = (async () => {
        ...
        // sur le tenant PRINCIPAL -- `schema`/`transform` restent écrits
        // pour la forme "sans enveloppe", sans aucun changement pour la
        // quasi-totalité des appels existants. `multiTenant: true` laisse
        // passer l'enveloppe brute pour l'appelant qui la demande
        // explicitement (voir repositories/news.repository.ts::
        // listAcrossTenants). Fait ICI, avant la mise en cache ci-dessous,
        // pour que ce soit la donnée déjà repliée/brute (selon le mode)
        // qui soit resservie par le cache et le stale-while-revalidate --
        // jamais l'enveloppe non traitée.
        const scopedData = opts.multiTenant ? rawData : unwrapToPrimaryTenant(rawData);

        const processedData = opts.transform ? opts.transform(scopedData) : scopedData;
