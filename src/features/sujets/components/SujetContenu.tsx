...
export interface SujetContenuProps {
  sujet: Sujet;
}

export const SujetContenu: React.FC<SujetContenuProps> = ({ sujet }) => {
  return (
    <div className="bg-white dark:bg-[#1A1F4D] rounded-none p-3 sm:p-5 border border-gray-100 dark:border-gray-800 shadow-sm mb-3">
      ...
      {sujet.description &&
        sujet.contenu &&
        sujet.description.trim() !== sujet.contenu.trim() && (
      ...
      <div className="my-2">
        <RichTextViewer content={sujet.contenu || sujet.description} />
      </div>
