...
export interface SujetActionsBarProps {
  sujet: Sujet;
...
export const SujetActionsBar: React.FC<SujetActionsBarProps> = ({ sujet, onUpdate }) => {
    ...
    <div className="w-full bg-white dark:bg-[#1A1F4D] rounded-2xl p-4 border border-gray-100 dark:border-gray-800 shadow-sm mb-8 flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1">
        {(Object.keys(EMOJI_MAP) as TypeReaction[]).map((type) => {
          const count = sujet.stats.reactions[type] || 0;
          const isSelected = sujet.userReaction === type;
          return (
            <button
              key={type}
              onClick={() => react(type)}
      ...
      <Modal isOpen={isQrOpen} onClose={() => setIsQrOpen(false)} title="QR Code & Diffusion">
        <div className="flex flex-col items-center justify-center p-4 text-center">
          ...
          <p className="text-xs text-gray-500 mt-4 max-w-xs">
            Scannez ce QR Code pour accéder directement à ce sujet ou affichez-le dans vos amphithéâtres.
          </p>
        </div>
      </Modal>
