...
import { Sujet, TypeReaction } from '../../../types/global.types';
...
export interface SujetHeaderSharePointStyleProps {
  sujet: Sujet;
  onUpdate?: (updated: Sujet) => void;
...
export const SujetHeaderSharePointStyle: React.FC<SujetHeaderSharePointStyleProps> = ({
  sujet,
  onUpdate,
  onScrollToComments,
  onScrollToPolls,
}) => {
  const { react } = useSujetReactions(sujet.id, onUpdate);
  ...
  const handleShareCopy = async () => {
    ...
    if (success) {
      toast('success', 'Lien copié !', 'Le lien du sujet a été copié dans votre presse-papier.');
    }
  };
  ...
  const toggleFollow = () => {
    ...
    toast('info', nextState ? 'Abonnement activé' : 'Désabonné', nextState ? 'Vous recevrez les notifications pour ce sujet.' : 'Vous ne recevrez plus les notifications.');
  };

  const handleReportSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsReportOpen(false);
    toast('info', 'Signalement transmis', 'Merci d’aider à préserver un espace de débat sain.');
  };
  ...
  return (
    <div className="w-full rounded-none overflow-hidden shadow-sm border border-emerald-900/10 mb-2 bg-white dark:bg-[#101538] text-xs">
      ...
      <div className="bg-[#005a42] text-white px-1 sm:px-2 py-0.5 flex items-center gap-1.5 sm:gap-3 overflow-x-auto no-scrollbar text-[11px]">
        ...
        <Link to="/sujets" className="hover:underline opacity-90 hover:opacity-100 whitespace-nowrap">
          Tous les Sujets
        </Link>
        ...
        <div className="flex items-center gap-1.5 max-w-2xl">
          ...
          <div>
            <div className="flex items-center gap-1 text-[10px] text-emerald-200 uppercase tracking-wider font-semibold leading-none mb-0.5">
              <span>SUJET #{sujet.id.slice(-4).toUpperCase()}</span>
              <span>•</span>
              <span className="capitalize">{sujet.type}</span>
            </div>
      ...
      <div className="bg-white dark:bg-[#161B40] border-b border-gray-200 dark:border-gray-800 px-1.5 sm:px-3 py-0.5 flex flex-wrap items-center justify-between gap-1 text-gray-700 dark:text-gray-200 text-[11px]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          {sujet.sondages && sujet.sondages.length > 0 && (
            <button
              onClick={onScrollToPolls}
              className="flex items-center gap-1 px-1.5 py-0.5 rounded-none hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium transition-colors cursor-pointer"
            >
              <BarChart2 className="w-3 h-3 text-blue-600" />
              <span>Analyse ({sujet.sondages.length})</span>
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-gray-500 dark:text-gray-400 font-medium flex-wrap text-[10px]">
          <span className="hidden md:inline">
            Publié {formatDateFull(sujet.createdAt)}
          </span>
                ...
                {(Object.keys(EMOJI_MAP) as TypeReaction[]).map((type) => (
                  <button
                    ...
                    className={`p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-none transition-transform hover:scale-110 ${
                      sujet.userReaction === type ? 'bg-emerald-50 dark:bg-emerald-950 ring-1 ring-emerald-500' : ''
                    }`}
        ...
        <div className="absolute inset-0 p-3 sm:p-5 flex flex-col justify-end max-w-5xl">
          ...
          <div className="flex flex-wrap items-center gap-3 text-[11px] text-emerald-300">
            <span className="flex items-center gap-1.5 bg-black/40 px-2 py-0.5 rounded backdrop-blur-sm border border-white/10">
              <Avatar src={sujet.auteur.avatar} name={sujet.auteur.nomAffiche} size="sm" className="w-4 h-4 rounded-full" />
              <span>Par {sujet.auteur.nomAffiche}</span>
            </span>
            {sujet.organisation && (
      ...
      {/* 5. Bottom Teal Quick-Info Bar (Matching SharePoint bottom strip) */}
      <div className="bg-[#00785a] text-white p-2 sm:p-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
        <div className="flex items-start gap-1.5">
          ...
          <div>
            ...
            <div className="text-emerald-100 opacity-90 text-[10px] mt-0.5">
              {formatDateFull(sujet.createdAt)}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-1.5">
          ...
          <div>
            <div className="font-semibold leading-none">Statut du sujet</div>
            <div className="text-emerald-100 opacity-90 text-[10px] mt-0.5">
              {sujet.dateFin ? `Clôture le ${formatDateFull(sujet.dateFin)}` : 'Debat ouvert aux citoyens'}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-1.5">
          ...
          <div>
            ...
            <div className="text-emerald-100 opacity-90 text-[10px] mt-0.5">
              {sujet.stats.commentaires} commentaires • {sujet.stats.vues} vues
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-1.5">
          ...
          <div>
            ...
            <div className="text-emerald-100 opacity-90 text-[10px] mt-0.5">
              {sujet.province || 'Nationale & Interuniversitaire'}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Page Details Modal */}
      <Modal isOpen={showDetailsModal} onClose={() => setShowDetailsModal(false)} title="Détails du sujet">
        <div className="space-y-3 text-xs text-gray-700 dark:text-gray-300">
          ...
          <div>
            ...
            <span>{sujet.auteur.nomAffiche} ({sujet.auteur.role})</span>
          </div>
          <div>
            <span className="font-semibold text-gray-900 dark:text-white">Catégorie: </span>
            <span>{sujet.categorie.nom}</span>
          </div>
          <div>
            <span className="font-semibold text-gray-900 dark:text-white">Région / Province: </span>
            <span>{sujet.province || 'Toutes les provinces'}</span>
          </div>
          <div>
            <span className="font-semibold text-gray-900 dark:text-white">Nombre de réactions: </span>
            <span>{Object.values(sujet.stats.reactions).reduce((a, b) => a + b, 0)}</span>
          </div>
        </div>
      </Modal>

      {/* QR Modal */}
      <Modal isOpen={isQrOpen} onClose={() => setIsQrOpen(false)} title="QR Code du sujet">
        <div className="flex flex-col items-center justify-center p-3 text-center">
          <LienQrCode url={window.location.href} title={sujet.titre} />
          <p className="text-xs text-gray-500 mt-2">
            Scannez pour ouvrir directement ce sujet sur mobile.
          </p>
        </div>
      </Modal>

      {/* Report Modal */}
      <Modal isOpen={isReportOpen} onClose={() => setIsReportOpen(false)} title="Signaler ce Sujet">
        <form onSubmit={handleReportSubmit} className="space-y-3 text-xs">
          <p className="text-gray-600 dark:text-gray-300">
            Pour quel motif souhaitez-vous signaler ce sujet au comité de modération ?
          </p>
          <select
            value={reportReason}
